...
//-----------Ion current for UBC------------------
class IUBC{
	double tau, inf, alpha, beta;
	double K, a1, a2, b1, b2;
public:
	double d0, h0, H0;
	double itot, iT, ih, iL, iKIR, E_Ca, G_T, G_L, E_H, G_H, E_K, G_KIR;
	...
	void resetEL(double v, double G_M2) // constructor 
	{
		inf = 1. / (1.+ exp((v+79.2)/6.2));
		K = sqrt(1/inf -1 + 0.25) - 0.5;		
		tau = 8. + (211.4+exp((v+111.2)/5.) ) /( 18.7* (1+exp((v+82.)/3.5)) );
		a1 = 1/( tau * (1+K));
		b1 = a1 * K;
		tau = 8. + (211.4+exp((v+111.2)/5.) ) /( 0.5* (1+exp((v+82.)/4.5)) );
		a2 = 1/( tau * (1+K));
		b2 = a2 * K;
		d0 = b1*b2 / ( (a1+b1)*(a2+b2) - a1*b2) ;
		h0 = a1 / (a1+b1) * (1-d0);

		inf = 1. / (1.+ exp(-(v+61.)/7.));
		iT = G_T*inf*h0*(v - E_Ca);

  	    H0 = 1. /( 1.+ exp((v+105.)/5.5) );
		ih = G_H * H0* (v - E_H);

		inf = 1. / (1.+ exp(-(v+28.6)/8.4));
		iL = G_L*inf*(v - E_Ca);

		inf = 1 / (1+exp( (v - (-55.))/40.5));    
		iKIR = G_KIR *inf* (v - E_K);

		itot = iT + ih + iL + G_M2 * iKIR;

	} 

	void calc(double h, double H, double d, double &fh, double &fH, double &fd, double v, double GATE_M2);
};

void IUBC::calc(double h, double H, double d, double &fh, double &fH, double &fd, double v, double GATE_M2)
{
	//simple model
	/*
	inf = 1. / (1.+ exp((v+79.2)/6.2));
    ...
    */
	...
	a2 = 1/( tau * (1+K));
	b2 = a2 * K;
	fh = a1 *(1-h-d) - b1*h;    // Update h
    fd = b2 *(1-h-d) - a2*d;    // Update d
	//*/
	...
	//tau = 100. + 1000. * exp(-(-75.-v)*(-75.-v)/(15.*15.));
    //tau = 10. + (211.4+exp((v+111.2)/5.) ) /( 6.7* (1.+exp((v+82.)/3.5)) );
	tau = 100. + 90. /( exp((v+46.4)/109.3) + exp(-(v+71.6)/13.) ) ; 
	inf = 1. /( 1.+ exp((v+105.)/5.5) );
	fH  = (inf - H)/tau;   
	ih = G_H * H* (v - E_H);

	/*
	alpha = 0.4 * exp(-0.041 *(v+63.94) );
	beta =  0.51 * exp(0.028 *(v+63.94) );
	inf = alpha / (alpha+beta);
    tau = 1 / (alpha+beta);;
	fmKIR = (inf - mKIR)/tau;  
	iKIR = G_KIR *mKIR* (v - E_K);
	*/

	inf = 1 / (1+exp( (v - (-55.))/40.5));    
	iKIR = G_KIR *inf* (v - E_K);

	itot = iT + ih + iL + GATE_M2 * iKIR;
}


//-------------------Unipolar brush cell-----------------------------------------------
class UBCIFB2: public IUBC, public MGsoma, public MUsoma, public MGosoma{
	double taum; 
	double VON, VOFF; 
	static double V0, spkdur, refact; 
	double Iapp, Imax, lastspk, TimeCounter;

public:
	double iLeak, Cm ,G_l, E_l;
	int    Ca;  //Ca is spike number
	double spiketimes[SPKTIME], GATE_AMPA, GATE_M1, GATE_M2;
	...
	UBCIFB2(){
		...
		tauAHP  = 10.0;
		gAHPbar = GK;   // nS; peak of AHP current
		eAHP = -90.;
...
double UBCIFB2::spkdur=1., UBCIFB2::refact=2.;
double UBCIFB2::V0 = -67. ;

void UBCIFB2::calc(double x, double *y, double *f)
{
    q = (x- lastspk) - spkdur; //time since last spike ended
    TimeCounter -= DT;              
	...
	// Vm
	Inoise = (GetRandomDoub(1.0) -0.5 ) * Imax;
	//if ( GetRandomDoub(1.0) < DT * UBCRATE ) { 
	//	Inoise = -3000;  
	//}    

	//IT  = gT m h V - VT
	IUBC::calc(y[2], y[3], y[4], f[2], f[3], f[4], y[0], GATE_M2);

	// voltage-clamp
#if V_CLAMP
	y[0] = -70; // -75//mGluR2  -72//ex -70//inh
#else
#endif

#if CURRENT_CLAMP
	f[0] = ( -G_l*(y[0]-E_l) - y[1]*(y[0]-eAHP) - itot - Inoise + Iapp) /Cm; 
#else

    //ff = 20*1e-3 * ct / tmax;
    //Istim = 1 * sin( 2*pi*ff * ct ) ;

	//Iapp = 1 * sin( 0.35 * pow( x/1000., 3.0 ) );
	f[0] = ( -G_l*(y[0]-E_l) - y[1]*(y[0]-eAHP) - itot - Inoise + Iapp ) /Cm; 
	//f[0] = ( -G_l*(y[0]-E_l) - Inoise) / Cm; 
#endif
